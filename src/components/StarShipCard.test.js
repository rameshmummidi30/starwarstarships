...
it('renders without crashing', () => {
    ...
    ReactDOM.render(<StarShipCard StarShips={starships}/>, div);
    ReactDOM.unmountComponentAtNode(div);
});

it('no of items must be 10', () => {
    let starships = [
        {
        "name":"Executor",
        "model":"Executor-class star dreadnought",
    ...
    const { getByText } = render(<StarShipCard StarShips={starships}/>);
    expect(getByText('Executor')).toBeInTheDocument();
    expect(getByText('Sentinel-class landing craft')).toBeInTheDocument();
    ...
    expect(getByText('Slave 1')).toBeInTheDocument();
    expect(getByText('Imperial shuttle')).toBeInTheDocument();
    expect(getByText('EF76 Nebulon-B escort frigate')).toBeInTheDocument();
});
